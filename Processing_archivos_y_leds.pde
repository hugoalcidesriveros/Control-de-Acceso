...
User[] users = {
  new User("Alcides", "Iluminador", "UID: 8E-73-90-2", 50, 100, 150, 100,1),
  new User("Francisco", "Sonidista", "UID: B3-F-7E-10", 250, 100, 100, 100,2),
  new User("Pablo", "Electricista", "UID: B3-F-7E-14", 400, 100, 100, 100,3),
  new User("Rubén", "Directora", "UID: 5796692", 200, 250, 150, 100,4),
};
...
void setup() {
  ...
  try {
    FileWriter fw = new FileWriter("D:/Final de Final/Control-de-Acceso/Arduino y Processing/Processing_archivos_y_leds/trabajador.txt", true); // 'true' para añadir sin sobrescribir
...
void draw() {
  ...
  if (myPort.available() > 0) {
    ...
    if (inData != null) {
      ...
      String currentDate = nf(day(), 2) + "/" + nf(month(), 2) + "/" + year();
      // Procesar entrada/salida según el UID
      for (int i = 0; i < users.length; i++) {
        if (inData.contains(users[i].uid)) {
          users[i].processEntryExit(currentDate, currentTime);
          ...
          enviarValorAlArduino(users[i]); // Enviar valor al Arduino
...
void enviarValorAlArduino(User user) {
  if (myPort != null) {
    myPort.write(user.arduinoValue);
    println("Valor enviado al Arduino: " + user.arduinoValue);
  }
}
...
class User {
  ...
  User(String name, String role, String uid, int x, int y, int w, int h, int arduinoValue) {
    this.name = name;
    this.role = role;
    this.uid = uid;
    this.x = x;
...
void processEntryExit(String currentDate, String currentTime) {
    if (!isEntry) {
      // Registrar entrada
      writer.println("Nombre: " + name + " (" + role + ") - Fecha: " + currentDate + " - Horario de entrada: " + currentTime);
      writer.flush();
      println("Datos guardados: Nombre: " + name + " (" + role + ") - Fecha: " + currentDate + " - Horario de entrada: " + currentTime);
    } else {
      // Registrar salida
      writer.println("Nombre: " + name + " (" + role + ") - Fecha: " + currentDate + " - Horario de salida: " + currentTime);
      writer.flush();
      println("Datos guardados: Nombre: " + name + " (" + role + ") - Fecha: " + currentDate + " - Horario de salida: " + currentTime);
    }
    isEntry = !isEntry; // Alternar estado
}
}
